...
--no, not all the files since .gitignore is specfeid as hidden so git add * will  add all but .gitignore--
...
1. head will be on c4 with experiment.
2. expirment will be integrated with master on c4 and c4 will stay the master.
3. head will become on c5.
4. noting will happen since c5 is the merged of c3 and c4.
...
yes since everything is up to date and when we created code.py, we can now use -a with no conflects.
